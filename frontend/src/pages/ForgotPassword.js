...
const ForgotPassword = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let data;
      try { data = await res.json(); } catch { throw new Error("Server error. Please check your backend is running."); }
    ...
    <div className="min-h-screen bg-gray-900 flex items-center justify-center px-4 py-8">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-400 text-sm mt-2">Enter your registered email and we'll send a reset link</p>
        </div>
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-2xl p-6 sm:p-8">
          {success ? (
            ...
            <div className="space-y-4">
              ...
              <Link to="/login"
                className="block text-center text-sm text-gray-500 hover:text-gray-300 transition mt-2">
                ← Back to Login
              </Link>
            </div>
          )}
