...
const Profile = () => {
  ...
  const handleChangePassword = async () => {
    ...
    if (passwords.newPassword.length < 6) return setPwError("New password must be at least 6 characters");
    setPwLoading(true);
    try {
      ...
      <div className="p-4 sm:p-6 max-w-2xl mx-auto space-y-4">
        ...
        <div className="bg-white rounded-2xl shadow p-6 flex flex-col items-center gap-3">
          <div className="relative">
            <div className="w-28 h-28 rounded-full overflow-hidden bg-gray-200 border-4 border-red-100 shadow">
              {form.profilePicture ? (
            ...
            <button onClick={() => fileRef.current.click()}
              className="absolute bottom-0 right-0 bg-red-600 text-white w-8 h-8 rounded-full flex items-center justify-center text-sm shadow-lg hover:bg-red-700">
              📷
            </button>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="text-gray-500 text-sm capitalize">{user?.role} • {user?.department}</p>
            <p className="text-gray-400 text-xs mt-1">{user?.matricNumber}</p>
          </div>
        </div>
          ...
          {["profile", "password"].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition ${
                activeTab === tab ? "bg-red-600 text-white shadow" : "bg-white text-gray-600 shadow"
              }`}>
        ...
        {activeTab === "profile" && (
          <div className="bg-white rounded-2xl shadow p-5 space-y-4">
            {success && <div className="bg-green-50 text-green-700 text-xs p-3 rounded-xl border border-green-200">{success}</div>}
            {error && <div className="bg-red-50 text-red-600 text-xs p-3 rounded-xl border border-red-200">{error}</div>}
            {[
              ...
              { name: "address", label: "Home/Hostel Address", placeholder: "Block A Room 12" },
        ...
        {activeTab === "password" && (
          <div className="bg-white rounded-2xl shadow p-5 space-y-4">
            {pwSuccess && <div className="bg-green-50 text-green-700 text-xs p-3 rounded-xl border border-green-200">{pwSuccess}</div>}
            {pwError && <div className="bg-red-50 text-red-600 text-xs p-3 rounded-xl border border-red-200">{pwError}</div>}
            {[
              ...
              <div key={name}>
                ...
                <div className="relative">
                  <input type={showPasswords[key] ? "text" : "password"}
                    ...
                    className="w-full border border-gray-300 rounded-xl px-3 py-2.5 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-red-500"
                    placeholder="••••••••" />
                  <button type="button"
