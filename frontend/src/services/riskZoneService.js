...
export const getRiskZones = async (token) => {
  const res = await fetch(`${BASE_URL}/riskzones`, { headers: { Authorization: `Bearer ${token}` } });
  if (!res.ok) throw new Error("Failed to fetch risk zones");
  return res.json();
};

export const autoClassifyAll = async (token) => {
  const res = await fetch(`${BASE_URL}/riskzones/auto-classify`, { headers: { Authorization: `Bearer ${token}` } });
  if (!res.ok) throw new Error("Auto-classification failed");
  return res.json();
};

export const deleteRiskZone = async (id, token) => {
  const res = await fetch(`${BASE_URL}/riskzones/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
